...
class ModelConfig:
    ...
    moe_capacity_factor: float = 1.0        # Expert Choice guarantees balance; no overalloc needed
...
# Requires ~12GB VRAM across 2x T4 (Kaggle T4x2 with DDP)
# Total Parameters: ~838M | Active Parameters per token: ~380M
# hidden=768, layers=16, heads=12, kv_heads=4, 8 experts + shared
FLAGSHIP_700M = ModelConfig(
    ...
    num_mtp_heads=0,
    max_seq_len=2048,      # 2048 halves activation VRAM vs 4096
    block_size=2048,
    rope_theta=10_000.0,
    rope_scaling_factor=1.0,
    rope_scaling_type="yarn",
